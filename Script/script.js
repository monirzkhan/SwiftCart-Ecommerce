...
const categoriesContainer=document.getElementById('product-categories');
    ...
    <button class="btn py-4 bg-white shadow-sm rounded-lg category-btn active" id="all-btn">
    All 
    </button> `;
...
const displayProductDetails=(product)=>{
    ...
    <div class="card bg-base-100 w-96 py-2 shadow-lg ">
        ...
        <div class="btn  bg-white py-4 w-5/12 rounded-lg"><i class="fa-solid fa-bag-shopping"></i>Buy Now</div>
        <div class="btn btn-primary w-5/12 rounded-lg"><i class="fa-solid fa-cart-shopping"></i>Add To Cart</div>
        </div>
        </div>
        </div> 
        `;
...
<div class="card bg-base-100 w-96 h-[420px] shadow-lg ">
        ...
        <p class="text-bold text-2xl text-left">$ ${product.price}</p>
        <div class="card-actions justify-between">
        <div onclick="loadProductDetails(${product.id})" class="btn  bg-white py-4 w-5/12 rounded-lg"><i class="fa-regular fa-eye"></i>Details</div>
...
const loadProductTrending=async(id)=>{
    const url=`https://fakestoreapi.com/products/${id}`;
    const res= await fetch(url);
    const data= await res.json();
    displayTrendingProductDetails(data);
}
const displayTrendingProductDetails=(product)=>{
    console.log(product);
...
<div class="card bg-base-100 w-96 h-[420px] shadow-lg ">
        ...
        <p class="text-bold text-2xl text-left">$ ${product.price}</p>
        <div class="card-actions justify-between">
        <div onclick="loadProductTrending(${product.id})" class="btn bg-white py-4 w-5/12 rounded-lg"><i class="fa-regular fa-eye"></i>Details</div>
        <div class="btn btn-active btn-primary w-5/12 rounded-lg"><i class="fa-solid fa-cart-shopping"></i>Add</div>
        </div>
        </div>
        </div>
